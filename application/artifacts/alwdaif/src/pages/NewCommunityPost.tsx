...
import {
  ...
  Palette, Type, ChevronDown, ArrowRight, Send
} from 'lucide-react';
...
function CommunityEditor({ content, onChange }: { content: string; onChange: (v: string) => void }) {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({ placeholder: 'اكتب محتوى موضوعك هنا... شارك تجربتك أو سؤالك مع المجتمع' }),
    ],
    content,
  ...
  const TB = ({ onClick, active, title, children }: { onClick: () => void; active?: boolean; title: string; children: React.ReactNode }) => (
    <button
      ...
      title={title}
      className={cn(
        'h-8 w-8 flex items-center justify-center rounded-lg transition-colors text-foreground/60 hover:text-foreground hover:bg-accent',
    ...
    <div className="border border-border rounded-2xl overflow-hidden bg-card">
      ...
      <div ref={toolbarRef} className="border-b border-border p-2 flex flex-wrap gap-0.5 items-center bg-muted/30">
        ...
        <div className="relative">
          <button
            type="button"
            onClick={() => { setShowFontFamily(!showFontFamily); setShowFontSize(false); setShowColor(false); }}
            className="h-8 px-2 flex items-center gap-1 text-foreground/60 hover:text-foreground hover:bg-accent rounded-lg text-sm transition-colors"
            title="نوع الخط"
          >
            ...
            <div className="absolute top-full right-0 mt-1 bg-card border border-border rounded-xl shadow-lg z-50 min-w-[150px] p-1">
              {arabicFonts.map(f => (
                <button key={f.name} type="button" onClick={() => { editor.chain().focus().setFontFamily(f.name).run(); setShowFontFamily(false); }}
                  className="w-full px-3 py-1.5 text-right text-foreground/70 hover:bg-accent rounded-lg text-sm" style={{ fontFamily: f.name }}>
                  {f.label}
                </button>
              ))}
            </div>
        ...
        <div className="relative">
          <button
            type="button"
            onClick={() => { setShowFontSize(!showFontSize); setShowFontFamily(false); setShowColor(false); }}
            className="h-8 px-2 flex items-center gap-1 text-foreground/60 hover:text-foreground hover:bg-accent rounded-lg text-sm transition-colors min-w-[48px] justify-center"
            title="حجم الخط"
          >
            <span>16</span>
            <ChevronDown className="h-3 w-3" />
            ...
            <div className="absolute top-full right-0 mt-1 bg-card border border-border rounded-xl shadow-lg z-50 p-1 max-h-48 overflow-y-auto min-w-[60px]">
              {fontSizes.map(s => (
                <button key={s} type="button" onClick={() => { editor.chain().focus().setFontSize(`${s}px`).run(); setShowFontSize(false); }}
                  className="w-full px-3 py-1 text-center text-foreground/70 hover:bg-accent rounded-lg text-sm">
                  {s}
                </button>
              ))}
            </div>
        ...
        {/* Link & Image */}
        <TB
          onClick={() => { const url = window.prompt('أدخل الرابط:'); if (url) editor.chain().focus().extendMarkRange('link').setLink({ href: url }).run(); }}
          active={editor.isActive('link')} title="رابط"
        ><LinkIcon className="h-4 w-4" /></TB>
        <TB
          onClick={() => { const url = window.prompt('أدخل رابط الصورة:'); if (url) editor.chain().focus().setImage({ src: url }).run(); }}
          title="صورة"
        ><ImageIcon className="h-4 w-4" /></TB>
      </div>
...
export default function NewCommunityPost() {
  usePageTitle("موضوع جديد - المجتمع");
  ...
  useEffect(() => {
    if (authData && !authData.authenticated) {
      localStorage.setItem("returnAfterLogin", "/community/new-post");
      setLocation("/login");
    }
  ...
  const createMutation = useMutation({
    mutationFn: async (data: { title: string; content: string; categoryId: number }) => {
      ...
      if (!res.ok) throw new Error("فشل نشر الموضوع");
      return res.json();
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/community/posts"] });
      toast({ title: "تم النشر بنجاح!", description: "تم نشر موضوعك في المجتمع" });
      setLocation(data?.id ? `/community/post/${data.id}` : "/community");
    },
    onError: () => {
      toast({ title: "خطأ", description: "فشل نشر الموضوع، حاول مرة أخرى", variant: "destructive" });
    },
  });
  ...
  const handleSubmit = () => {
    ...
    createMutation.mutate({ title: title.trim(), content, categoryId: parseInt(categoryId) });
  };
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl" dir="rtl">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          >
            ...
            <span className="font-medium">العودة للمجتمع</span>
          </button>
          <h1 className="text-xl md:text-2xl font-black text-foreground">إنشاء موضوع جديد</h1>
          <Button
            onClick={handleSubmit}
            disabled={createMutation.isPending}
            className="gap-2 flex-row-reverse font-bold rounded-xl shadow-lg shadow-primary/20"
            data-testid="button-publish"
          >
            <Send className="h-4 w-4" />
            {createMutation.isPending ? "جارٍ النشر..." : "نشر الموضوع"}
          </Button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              placeholder="اكتب عنواناً واضحاً ومعبّراً عن موضوعك..."
              value={title}
          ...
          <div className="space-y-2">
            ...
            <Select value={categoryId} onValueChange={setCategoryId}>
              <SelectTrigger className="h-12 rounded-xl border-border focus:border-primary" data-testid="select-category">
                <SelectValue placeholder="اختر القسم المناسب لموضوعك" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <CommunityEditor content={content} onChange={setContent} />
          </div>

          {/* Tips */}
          <div className="bg-primary/5 border border-primary/15 rounded-2xl p-4 text-sm text-muted-foreground space-y-2">
            <p className="font-bold text-foreground/80 mb-2">💡 نصائح لموضوع مميز:</p>
            <ul className="space-y-1.5 list-none">
              <li className="flex items-start gap-2"><span className="text-primary mt-0.5">•</span> استخدم عنواناً واضحاً يعبّر عن محتوى الموضوع</li>
              <li className="flex items-start gap-2"><span className="text-primary mt-0.5">•</span> فصّل أفكارك بالعناوين والقوائم لتسهيل القراءة</li>
              <li className="flex items-start gap-2"><span className="text-primary mt-0.5">•</span> أضف تجربتك الشخصية لتضيف قيمة للمجتمع</li>
              <li className="flex items-start gap-2"><span className="text-primary mt-0.5">•</span> اختر القسم الصحيح لكي يصل موضوعك للجمهور المناسب</li>
            </ul>
          </div>
          ...
          <div className="flex gap-4 justify-end pt-2">
            ...
            <Button
              onClick={handleSubmit}
              disabled={createMutation.isPending}
              className="h-12 px-8 rounded-xl font-bold gap-2 flex-row-reverse shadow-lg shadow-primary/20"
